...
func GetIACSupplier(configs []config.SupplierConfig, library *terraform.ProviderLibrary, backendOpts *backend.Options, progress output.Progress) (resource.Supplier, error) {
	...
	for _, config := range configs {
		...
		case state.TerraformStateReaderSupplier:
			supplier, err = state.NewReader(config, library, backendOpts, progress)
		default:
			return nil, errors.Errorf("Unsupported supplier '%s'", config.Key)
		}
